...
if ! [[ -f serverstarter-0.3.3.jar ]]; then
	...
	if command -v wget &> /dev/null; then
		...
		wget -O serverstarter-0.3.3.jar "${URL}"
	elif command -v curl &> /dev/null; then
		echo "DEBUG: (curl) Downloading ${URL}"
		curl -o serverstarter-0.3.3.jar "${URL}"
	else
...
java $JAVA_FLAGS $JVM_OPTS -Dlog4j.configurationFile=log4j2_112-116.xml -jar serverstarter-0.3.3.jar
